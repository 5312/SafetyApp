<template>
	...
	<!-- <view class="wrap">
		<view class="top"></view>
		<view class="content">
			<view class="title">安全管理信息平台</view>
			<input class="u-border-bottom" type="text" v-model="tel" placeholder="请输入账号" />
			<input class="u-border-bottom" type="password" v-model="password" placeholder="请输入密码" />
			<view class="tips">未注册的请联系管理员注册账号</view>
				...
				this.$http.post('?type=login', {
					...
					setTimeout(() => {
						if (res.data.code == '200') {
							...
							try {
								uni.setStorageSync('setUser', res.data.us); //存入缓存
							} catch (e) {
								// error
							}
							uni.showToast({
